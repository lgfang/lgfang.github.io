...

  ...
  infoView: function (sec, skip_show_section)
  {
    ...
    if(t.POSTAMBLE) OrgNode.hideElement(t.POSTAMBLE); // lgfang: changed showElement to hideElement
    if(!skip_show_section)
      t.showSection(sec);
    window.scrollTo(0, 0);
  },
  ...
  slideView: function (sec, skip_show_section)
  {
    ...
    OrgNode.showElement(t.TITLE); // lgfang: comment out this to NOT should title on every slide
    OrgNode.showElement(t.WINDOW);
    t.ROOT.hideAllChildren();
    OrgNode.hideElement(t.TOC.DIV);
